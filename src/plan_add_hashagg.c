...
initialize_custom_estimate_func_info()
{
	...
	HASHCTL		hashctl = {
		.keysize = sizeof(Oid),
		.entrysize = sizeof(CustomEstimateForFunctionInfo)
	};
	...
	for (i = 0; i < _MAX_HASHAGG_FUNCTIONS; i++)
	{
		...
		CustomEstimateForFunctionInfo *hash_entry;
		bool		function_found = false;
		bool		hash_found;
		FuncCandidateList funclist = FuncnameGetCandidates(list_make1(makeString(def.function_name)),
		...
		if (!function_found)
			elog(ERROR, "oid lookup failed for the function %s with %d args",
		...
		Assert(!hash_found);
		memcpy(hash_entry, &(custom_estimate_func_info[i]), sizeof(CustomEstimateForFunctionInfo));
	}
}
...
get_custom_estimate_func_info(Oid function_oid)
{
	...
	return hash_search(custom_estimate_func_hash, &function_oid, HASH_FIND, NULL);
}
...
custom_group_estimate_integer_division(PlannerInfo *root, Oid opno, Node *left, Node *right)
{
	...
	if (strlen(function_name) == 1 && function_name[0] == '/' && IsA(right, Const))
	{
		Const	   *c = (Const *) right;
...
custom_group_estimate_expr(PlannerInfo *root, Node *expr, double path_rows)
{
	...
	{
		FuncExpr   *custom_group_estimate_func = (FuncExpr *) expr;
