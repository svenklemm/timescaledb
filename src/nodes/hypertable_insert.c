...
static TupleTableSlot *ExecPrepareTupleRouting(ChunkDispatchState *cds, ModifyTableState *mtstate,
...
static ModifyTableState * ExecInitModifyHyperTable(ModifyTable *node, EState *estate, int eflags);
...
//	ps = ExecInitNode(&state->mt->plan, estate, eflags);
  ps = ExecInitModifyHyperTable(&state->mt->plan, estate, eflags);
...
#else
	chunk_dispatch_states = get_chunk_dispatch_states(outerPlanState(mtstate));
#endif
...
#else
	Assert(list_length(mtpath->resultRelations) == 1);
#endif
...
#else
	mtpath->subpath = linitial(subpaths);
#endif
 ...
 */
static TupleTableSlot *
ExecInsert(ChunkDispatchState *cds, ModifyTableState *mtstate, ResultRelInfo *resultRelInfo,
		   TupleTableSlot *slot, TupleTableSlot *planSlot, EState *estate, bool canSetTag)
{
	...
	if (proute)
	{
		...
		slot = ExecPrepareTupleRouting(cds,
									   mtstate,
									   estate,
									   proute,
									   resultRelInfo,
 ...
 */
static bool
ExecCrossPartitionUpdate(ModifyTableState *mtstate, ResultRelInfo *resultRelInfo,
						 ItemPointer tupleid, HeapTuple oldtuple, TupleTableSlot *slot,
						 TupleTableSlot *planSlot, EPQState *epqstate, bool canSetTag,
						 TupleTableSlot **retry_slot, TupleTableSlot **inserted_tuple)
{
	EState *estate = mtstate->ps.state;
	...
	*retry_slot = NULL;
	...
	if (((ModifyTable *) mtstate->ps.plan)->onConflictAction == ONCONFLICT_UPDATE)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 ...
				 errdetail("The result tuple would appear in a different partition than the "
						   "original tuple.")));
	...
	if (mtstate->mt_partition_tuple_routing == NULL)
	{
		...
		mtstate->mt_partition_tuple_routing = ExecSetupPartitionTupleRouting(estate, rootRel);
	...
	//	*inserted_tuple =
	//		ExecInsert(mtstate, mtstate->rootResultRelInfo, slot, planSlot, estate, canSetTag);
		   ...
		   EState *estate, bool canSetTag)
{
	...
	else if (resultRelInfo->ri_FdwRoutine)
	{
		 ...
		 */
		slot =
			resultRelInfo->ri_FdwRoutine->ExecForeignUpdate(estate, resultRelInfo, slot, planSlot);
	...
	lreplace:;
		...
		if (resultRelInfo->ri_NumIndices > 0 && update_indexes)
			recheckIndexes =
				ExecInsertIndexTuples(resultRelInfo, slot, estate, true, false, NULL, NIL);
	}
 ...
 */
static bool
ExecOnConflictUpdate(ModifyTableState *mtstate, ResultRelInfo *resultRelInfo,
					 ItemPointer conflictTid, TupleTableSlot *planSlot,
					 TupleTableSlot *excludedSlot, EState *estate, bool canSetTag,
					 TupleTableSlot **returning)
{
 ...
 */
static TupleTableSlot *
ExecPrepareTupleRouting(ChunkDispatchState *cds, ModifyTableState *mtstate, EState *estate,
						PartitionTupleRouting *proute, ResultRelInfo *targetRelInfo,
						TupleTableSlot *slot, ResultRelInfo **partRelInfo)
{
	...
	if (mtstate->mt_transition_capture != NULL)
	{
		...
		has_before_insert_row_trig =
			(partrel->ri_TrigDesc && partrel->ri_TrigDesc->trig_insert_before_row);
...
ExecModifyTable(PlanState *pstate)
{
	...
	// ChunkDispatchState *cds = linitial(get_chunk_dispatch_states(subplanstate));
	ChunkDispatchState *cds;

	// Assert(ts_chunk_dispatch_is_state(subplanstate));
	...
	for (;;)
	{
		...
		switch (operation)
		{
			case CMD_INSERT:
				...
				slot =
					ExecInsert(cds, node, resultRelInfo, slot, planSlot, estate, node->canSetTag);
				break;
			case CMD_UPDATE:
				/* Initialize projection info if first time for this table */
				if (unlikely(!resultRelInfo->ri_projectNewInfoValid))
 ...
 */
ModifyTableState *
ExecInitModifyHyperTable(ModifyTable *node, EState *estate, int eflags)
{
	ModifyTableState *mtstate;
	Plan	   *subplan = outerPlan(node);
	CmdType		operation = node->operation;
	 ...
	 */
	if (node->returningLists && &node->returningLists->elements[0] != NULL)
	{
		TupleTableSlot *slot;
		ExprContext *econtext;
